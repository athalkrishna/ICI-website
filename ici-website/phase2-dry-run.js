...
const top10BodyPatterns = inventory.body
  ...
  .filter(className => {
    // Exclude manual review flags
    if (className.includes('font-mono')) return false; // Quote/callout blocks
    if (className.includes('text-red-600')) return false; // Form error messages
    if (className.includes('text-body-hero')) return false; // Hero section typography
    return true;
  })
  .slice(0, 10);
...
fs.writeFileSync('phase2-dry-run-output.txt', diffs.join('\n'));
console.log('Phase 2 Dry run complete. Found ' + matchedCount + ' matches.');
