...
export const metadata: Metadata = {
  title: 'Alumni | International Coaching Institute',
}

export default async function AlumniPage() {
  const content = await getPageContent('alumni')
      ...
      <div className="max-w-[1440px] mx-auto px-4 lg:px-8">
        <AnimatedSection className="max-w-4xl mx-auto text-center mb-16">
          ...
          <h1 className="font-display text-5xl md:text-6xl font-bold text-navy-900 mb-8 leading-tight">
            {content.heading || 'Once an ICI coach, always part of ICI'}
          </h1>
          <p className="font-body text-xl text-gray-700 leading-relaxed">
            {content.body || 'The credential was a milestone, not an exit. Our alumni stay connected for the things that make a long coaching career sustainable...'}
          </p>
        </AnimatedSection>
        <AnimatedSection delay={0.2} className="max-w-4xl mx-auto bg-white p-10 text-center rounded-3xl shadow-sm border border-gray-100">
          <a href="/login" className="btn-primary inline-block">Log in to Alumni Network</a>
        </AnimatedSection>
      </div>
    </div>
  )
