...
export default function ApplyCTA({ content = {} }: ApplyCTAProps) {
  return (
    <Section spacing="standard" className="bg-brand-gold-gradient text-brand-navy-900 relative overflow-hidden">
      <div className="absolute inset-0 bg-hero-pattern opacity-10 mix-blend-overlay" aria-hidden />
      <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link href={cmsField(content, 'cta_primary_button_link', '/apply')} className="inline-flex items-center justify-center gap-2 px-8 py-4 min-h-[44px] bg-brand-navy-700 hover:bg-brand-navy-800 text-white font-sans font-semibold rounded-lg transition-colors">
            {cmsField(content, 'cta_primary_button_text', 'Start Your Application')}
            <ArrowRight size={18} aria-hidden />
          </Link>
          <Link href={cmsField(content, 'cta_secondary_button_link', '/admissions/contact')} className="inline-flex items-center justify-center gap-2 px-8 py-4 min-h-[44px] border-2 border-brand-navy-700 text-brand-navy-700 hover:bg-brand-navy-700 hover:text-white font-sans font-semibold rounded-lg transition-colors">
